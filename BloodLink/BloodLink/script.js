...
const bloodInventory = [
  {group:"A+",units:18,expiry:"2 units expiring in 4 days",status:"available"},
  ...
  {group:"O+",units:24,expiry:"3 units expiring in 2 days",status:"available"},
  {group:"O-",units:1,expiry:"Critical stock",status:"critical"}
];

const donors = [
  {name:"Rahul",group:"O+",distance:"2.9 km",availability:"Available",verification:"Verified",cooldown:"Eligible"},
  {name:"Ayesha",group:"O+",distance:"4.2 km",availability:"Available",verification:"Verified",cooldown:"Eligible"},
  {name:"Arjun",group:"A+",distance:"3.6 km",availability:"Available",verification:"Verified",cooldown:"Eligible"},
  {name:"Maya",group:"B+",distance:"5.1 km",availability:"Available",verification:"Verified",cooldown:"Eligible"},
  {name:"Dev",group:"AB+",distance:"6.8 km",availability:"Available",verification:"Verified",cooldown:"Eligible"}
];

const bloodBanks = [
  {name:"CityCare Blood Bank",group:"O+",units:8,distance:"2.4 km",score:96},
  {name:"LifeLine Blood Bank",group:"O+",units:5,distance:"4.1 km",score:89},
  {name:"Verified Donor Network",group:"O+",units:1,distance:"5.7 km",score:82}
];

const notifications = [
  {icon:"🚨",title:"Critical blood request received",text:"Demo request BL-1024 needs urgent coordination.",unread:true},
  {icon:"🩸",title:"O+ units available nearby",text:"CityCare Blood Bank has simulated availability.",unread:true},
  {icon:"✅",title:"CityCare Blood Bank confirmed request",text:"Confirmation is simulated for this prototype.",unread:false},
  {icon:"⚠",title:"A+ inventory is low",text:"4 simulated units currently available.",unread:false},
  {icon:"⏳",title:"3 units approaching expiry",text:"O+ units are flagged for expiry monitoring.",unread:false}
];

const pageTitles = {
  dashboard:"Hospital Dashboard", emergency:"Emergency Blood Request", inventory:"Blood Inventory",
  matching:"Smart Matching", notifications:"Notifications", history:"Request History", donors:"Donor Network", admin:"Admin Dashboard"
};
...
function initials(name){
  return name.split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join("").toUpperCase() || "BL";
}
...
function roleLabel(role){ return ({hospital:"Hospital",bloodbank:"Blood Bank",donor:"Donor",admin:"Admin"})[role] || "Demo User"; }

function renderPage(page){
  state.currentPage = page;
  document.querySelectorAll(".nav-item[data-page]").forEach(b=>b.classList.toggle("active",b.dataset.page===page));
  if(state.role==="admin" && page==="dashboard") $("topTitle").textContent = "Admin Dashboard";
  else $("topTitle").textContent = pageTitles[page] || "BloodLink";
  const content = $("pageContent");
  if(state.role==="admin" && page==="dashboard") content.innerHTML = adminPage();
...
function statusLabel(s){ return s==="available"?"Available":s==="low"?"Low":"Critical"; }

function stats(){
  const total = bloodInventory.reduce((a,b)=>a+b.units,0);
  return {total, critical:bloodInventory.filter(x=>x.status==="critical").length, active:state.request&&!state.resolved?1:0};
}
...
function coordinationPage(){
  ...
  const steps = [
    ["Request Created",true],["Matching Completed",true],["Resource Found",true],
    ["Notification Sent",true],["Blood Collection",state.confirmed],["Request Resolved",state.resolved]
  ];
...
function resolvePage(){
  ...
  const steps=["Request Created","Resource Matched","Resource Notified","Resource Confirmed","Blood Received","Request Resolved"];
...
function adminPage(){
  ...
  <div class="admin-grid"><div class="section"><div class="section-title"><h3>Inventory Status</h3><span>8 blood groups</span></div><div class="bars">${[["Available",62],["Low",25],["Critical",13]].map(x=>`<div class="bar-line"><span>${x[0]}</span><div class="bar"><i style="width:${x[1]}%"></i></div><b>${x[1]}%</b></div>`).join("")}</div></div>
  <div class="section"><div class="section-title"><h3>Verified Network</h3><span>Demo entities</span></div><div class="bars">${[["Blood Banks",78],["Donors",62],["Hospitals",44]].map(x=>`<div class="bar-line"><span>${x[0]}</span><div class="bar"><i style="width:${x[1]}%"></i></div><b>${x[1]}%</b></div>`).join("")}</div></div></div>
...
function openRequest(){ $("requestModal").classList.remove("hidden"); }
function closeModal(id){ $(id).classList.add("hidden"); }

function createRequest(data){
  if(!data.group || !data.units || Number(data.units)<1 || !data.location) { toast("Check the form","Please complete the required fields.","error"); return; }
  state.request={id:"BL-1024",group:data.group,units:Number(data.units),priority:data.priority,location:data.location,notes:data.notes,matchesReady:false};
  state.resolved=false; state.confirmed=false; state.selectedMatch=null;
  closeModal("requestModal");
  toast("Emergency request created",`#${state.request.id} · ${data.group} · ${data.units} units`);
  renderPage("matching");
  setTimeout(()=>{state.request.matchesReady=true;renderPage("matching");toast("Smart matching complete","Top demo match scored 96%.");},1400);
}

function bindDynamicButtons(){
  document.querySelectorAll("[data-page]").forEach(b=>b.onclick=()=>renderPage(b.dataset.page));
  document.querySelectorAll('[data-action="open-request"]').forEach(b=>b.onclick=openRequest);
  document.querySelectorAll('[data-action="select-match"]').forEach(b=>b.onclick=()=>{
    state.selectedMatch="CityCare Blood Bank"; state.currentPage="coordination"; $("topTitle").textContent="Notify & Coordinate";
    $("pageContent").innerHTML=coordinationPage(); bindDynamicButtons();
  });
  document.querySelectorAll('[data-action="confirm-resource"]').forEach(b=>b.onclick=()=>{
    state.confirmed=true; toast("Resource confirmed","CityCare Blood Bank confirmation simulated.");
    $("pageContent").innerHTML=coordinationPage(); bindDynamicButtons();
  });
  document.querySelectorAll('[data-action="go-resolve"]').forEach(b=>b.onclick=()=>{state.currentPage="resolve";$("topTitle").textContent="Blood Request Resolution";$("pageContent").innerHTML=resolvePage();bindDynamicButtons()});
  document.querySelectorAll('[data-action="resolve"]').forEach(b=>b.onclick=()=>{
    state.resolved=true; state.notificationCount=Math.max(0,state.notificationCount-1); $("navNotif").textContent=state.notificationCount;
    toast("Request resolved","Emergency request successfully resolved.");
    $("topTitle").textContent="Blood Request Resolution"; $("pageContent").innerHTML=resolvePage(); bindDynamicButtons();
  });
  const inline=$("inlineRequestForm");
  if(inline) inline.onsubmit=e=>{e.preventDefault();createRequest({group:$("iBloodGroup").value,units:$("iUnits").value,priority:$("iPriority").value,location:$("iLocation").value,notes:$("iNotes").value})};
}

document.querySelectorAll(".role-card").forEach(card=>{
  card.onclick=()=>{
    document.querySelectorAll(".role-card").forEach(c=>c.classList.remove("selected"));
    card.classList.add("selected"); state.role=card.dataset.role;
    $("roleGrid").classList.add("hidden"); $("loginForm").classList.remove("hidden");
    $("loginName").value = ({hospital:"CityCare Hospital",bloodbank:"CityCare Blood Bank",donor:"Demo Donor",admin:"BloodLink Admin"})[state.role];
  };
});

$("changeRole").onclick=()=>{$("loginForm").classList.add("hidden");$("roleGrid").classList.remove("hidden");document.querySelectorAll(".role-card").forEach(c=>c.classList.remove("selected"))};
$("loginForm").onsubmit=e=>{
  e.preventDefault(); state.user.name=$("loginName").value.trim()||"Demo User"; state.user.contact=$("loginContact").value.trim()||"demo@bloodlink.local"; showApp(); toast("Demo login successful",`Welcome, ${state.user.name}.`);
};
$("requestForm").onsubmit=e=>{e.preventDefault();createRequest({group:$("bloodGroup").value,units:$("unitsRequired").value,priority:$("priority").value,location:$("requestLocation").value,notes:$("requestNotes").value})};
document.querySelectorAll("[data-close]").forEach(b=>b.onclick=()=>closeModal(b.dataset.close));
$("requestModal").onclick=e=>{if(e.target===$("requestModal"))closeModal("requestModal")};
$("logoutBtn").onclick=()=>{state.role=null;state.request=null;state.resolved=false;state.confirmed=false;showLogin();toast("Logged out","Demo session ended.")};
$("notifBtn").onclick=()=>renderPage("notifications");
$("mobileMenu").onclick=()=>$("sidebar").classList.toggle("open");
document.addEventListener("click",e=>{const nav=e.target.closest(".nav-item[data-page]");if(nav)$("sidebar").classList.remove("open")});

// Allow navigation to coordination/resolution without exposing extra sidebar items.
const originalRenderPage = renderPage;
renderPage = function(page){
  if(page==="coordination"){ $("topTitle").textContent="Notify & Coordinate"; $("pageContent").innerHTML=coordinationPage(); bindDynamicButtons(); return; }
  if(page==="resolve"){ $("topTitle").textContent="Blood Request Resolution"; $("pageContent").innerHTML=resolvePage(); bindDynamicButtons(); return; }
  originalRenderPage(page);
};

window.addEventListener("keydown",e=>{if(e.key==="Escape")closeModal("requestModal")});
